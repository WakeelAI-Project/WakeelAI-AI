...
import { GoogleGeminiLanguageModel } from "../src/llm/google-adapter.js";
      ...
      global.fetch = jest.fn(async () => {
        ...
        if (callCount === 1) {
          return {
            ...
            status: 429,
            headers: new Headers({ "retry-after": "0.01" }),
            text: async () => JSON.stringify({ error: { message: "Resource has been exhausted" } }),
          };
        }
        return {
          ok: true,
          status: 200,
          json: async () => ({
            candidates: [{ content: { parts: [{ text: "Gemini Success response after retry" }] } }],
          }),
        };
      });

      const model = new GoogleGeminiLanguageModel({ apiKey: "test", modelName: "gemini-2.5-flash" });
      const result = await model.invoke("Hello");

      expect(callCount).toBe(2);
      expect(result.content).toBe("Gemini Success response after retry");
    });
