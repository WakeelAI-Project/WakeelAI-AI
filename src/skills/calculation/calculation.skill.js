...
const calculationSkill = {
  name: "calculation",
  description:
    "Performs deterministic mathematical operations such as addition, subtraction, multiplication, division, and percentage calculations. Do not use for generic conversation.",
...
User Request: "${message}"`);
      ...
      logger.info(
        `[CalculationSkill] Forwarding structured inputs to CalculationService`,
      );
      const calcResult = performCalculation(parsedInputs);

      if (!calcResult.success) {
        logger.warn(
          `[CalculationSkill] CalculationService reported failure: ${calcResult.explanation}`,
        );
        return {
          success: false,
          data: null,
      ...
      logger.error(
        `[CalculationSkill] Failed to execute calculation skill: ${error.message}`,
      );
      return {
        success: false,
        data: null,
        message:
          "An error occurred while attempting to parse or execute the calculation.",
        sources: [],
        action: null,
      };
    }
